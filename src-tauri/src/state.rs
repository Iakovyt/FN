...
pub struct AppState {
    ...
    pub db: Option<Db>,
    /// The winws process Hydra's own benchmark runner activates. Kept
    /// separate from `zapret` (the classic manual-strategy UI's process
    /// slot) so Hydra never fights the legacy control surface
    /// (`zapret_stop`, `set_gaming_mode`, ...) over the same handle.
    pub hydra: Mutex<ProcHandle>,
    pub hydra_active_strategy_id: Mutex<Option<String>>,
}
...
impl AppState {
    pub fn new(config: AppConfig, db: Option<Db>) -> Self {
        Self {
            ...
            db,
            hydra: Mutex::new(ProcHandle::default()),
            hydra_active_strategy_id: Mutex::new(None),
        }
    }
    ...
    pub fn hydra_running(&self) -> bool {
        self.hydra.lock().unwrap().is_running()
    }
