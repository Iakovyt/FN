...
- добавление доменов в `list-general.txt` из интерфейса;
...
Поле под переключателями принимает домен, IP-адрес или подсеть. Домены
сохраняются в `list-general.txt`, а IP-адреса и CIDR-подсети — в активный
IPSet. Если Zapret уже запущен, список применяется автоматически.
